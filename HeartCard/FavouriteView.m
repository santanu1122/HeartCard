...
{
    NSMutableArray *FavCardContainerArray;
    
    NSOperationQueue *OperationQueueForFav;
    
    GlobalMethod *Method;

    UIView  *AnimationBackView ,*BottomBarView, *CardLayer1, *CardLayer2, *CardLayer3, *CardLayer4, *fromNib1, *AnimationMainBackView, *PopupBackView;
    ...
    UILabel  *LblChooseAndPersonalize, *LblCreditsLine1, *LblAttachAnImage;
}
...
@synthesize ScreenViewFav, SVFav, Spinner, IsNewest;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        self=(IsIphone5)?[super initWithNibName:@"FavouriteViewBig" bundle:nil]:[super initWithNibName:@"FavouriteView" bundle:nil];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    [OperationQueueForFav addOperation:Operation];
}
...
-(void)PrepareScreens
{
    [self SetBackground:ScreenViewFav];
    [self AddTopBarToScreenView:ScreenViewFav withTitle:(IsNewest)?@"NEWEST":@"FAVORITES" WithFavIcon:(IsNewest)?YES:NO];
    [self AddButtonBarToScreenView:ScreenViewFav];
    FavCardContainerArray = [[NSMutableArray alloc] init];
    OperationQueueForFav=[[NSOperationQueue alloc] init];
...
-(void)goBack
{
    [self RemoveAllGestureFrom:ScreenViewFav];
...
-(void) FetchCards
{
    @try
    {
        NSString *fetchNewest=(IsNewest)?@"1":@"0";
        NSString *url=[NSString stringWithFormat:@"%@favorite.php?deviceId=%@&action=show&fetchNewest=%@", API, DeviceId, fetchNewest];
        NSLog(@"%@", url);
        ...
        if([getData length]>0)
        {
            ...
            {
                [FavCardContainerArray addObject:[[GlobalMethod alloc] initWithId:[var objectForKey:@"id"] withImageUrl:[var objectForKey:@"cardUrl"] withCardName:[var objectForKey:@"cardname"] withCounter:[[var objectForKey:@"id"] integerValue]]];
...
-(void)gotCards
{
    @try
    {
        unsigned CardContenerLength=[FavCardContainerArray count];
        if(CardContenerLength>0)
        {
            ...
            {
                Method=[FavCardContainerArray objectAtIndex:i];
                ...
                [[self SVFav] addSubview:CardLoaderView];
                ...
                [CardImageView setTag:[Method Counter]];
                ...
                [OperationQueueForFav addOperation:DownloadImageOperation];
                
                
                widthSize+=(IsIphone5)?260.0f:240.0f;
            }
            
            self.SVFav.contentSize = CGSizeMake(widthSize, 1);
            [[self SVFav] setShowsHorizontalScrollIndicator:NO];
        }
        else
        {
            UIAlertView *ShowAlert=[[UIAlertView alloc] initWithTitle:@"Sorry!!" message:@"No Record Found." delegate:self cancelButtonTitle:@"Ok" otherButtonTitles: nil];
            [ShowAlert show];
        }
    }
    @catch(NSException *exception)
...
{
    IsFavourite=(IsNewest)?FALSE:TRUE;
    ...
    [OperationQueueForFav addOperation:OperationFav];
    
    
    CGRect AnimationViewRect=[ScreenViewFav frame];
    AnimationBackView=[[UIView alloc] initWithFrame:AnimationViewRect];
    [AnimationBackView setBackgroundColor:[UIColor clearColor]];
    
    [ScreenViewFav addSubview:AnimationBackView];
...
-(void)PerformFav
{
    ...
    [OperationQueueForFav addOperation:DoOrUndo];
...
{
    NSLog(@"Open Custom Message editer");
    ...
    //[CustomMessageView setFont:[UIFont fontWithName:@"dearJoe 5 CASUAL trial" size:18.0f]];
...
//    [AnimationView addSubview:PopupBackView];
//    
//    CGRect BackUpperViewRect=CGRectMake(9.0f, 10.0f, 302.0f, 400.0f);
//    UIView *BackUpperView=[[UIView alloc] initWithFrame:BackUpperViewRect];
//    [BackUpperView setBackgroundColor:[UIColor whiteColor]];
//    
//    [PopupBackView addSubview:BackUpperView];
...
//    
//    [self AddMesssageSeperator:BackUpperView fromTop:40.0f];
//    [self AddMesssageSeperator:BackUpperView fromTop:135.0f];
//    [self AddMesssageSeperator:BackUpperView fromTop:225.0f];
//    [self AddMesssageSeperator:BackUpperView fromTop:315.0f];
//    
//    
//    CGRect Message1Rect=CGRectMake(5.0f, 70.0f, 290.0f, 80.0f);
//    UITextView *TextView1=[[UITextView alloc] initWithFrame:Message1Rect];
//    [TextView1 setTextAlignment:NSTextAlignmentCenter];
//    [TextView1 setText:StaticMessage1];
//    [TextView1 setFont:[UIFont fontWithName:@"Vollkorn-Regular" size:16.0f]];
//    [TextView1 setEditable:NO];
//    [TextView1 setScrollEnabled:NO];
//    [TextView1 setUserInteractionEnabled:YES];
//    [TextView1 setTextColor:UIColorFromRGB(0x776948)];
//    [BackUpperView addSubview:TextView1];
//    TapGesture=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectThisMessage:)];
//    [TextView1 addGestureRecognizer:TapGesture];
//    
//    CGRect Message2Rect=CGRectMake(5.0f, 160.0f, 290.0f, 80.0f);
//    UITextView *TextView2=[[UITextView alloc] initWithFrame:Message2Rect];
//    [TextView2 setTextAlignment:NSTextAlignmentCenter];
//    [TextView2 setText:StaticMessage2];
//    [TextView2 setFont:[UIFont fontWithName:@"Vollkorn-Regular" size:16.0f]];
//    [TextView2 setEditable:NO];
//    [TextView2 setScrollEnabled:NO];
//    [TextView2 setUserInteractionEnabled:YES];
//    [TextView2 setTextColor:UIColorFromRGB(0x776948)];
//    [BackUpperView addSubview:TextView2];
//    TapGesture=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectThisMessage:)];
//    [TextView2 addGestureRecognizer:TapGesture];
//    
//    CGRect Message3Rect=CGRectMake(5.0f, 250.0f, 290.0f, 80.0f);
//    UITextView *TextView3=[[UITextView alloc] initWithFrame:Message3Rect];
//    [TextView3 setTextAlignment:NSTextAlignmentCenter];
//    [TextView3 setText:StaticMessage3];
//    [TextView3 setFont:[UIFont fontWithName:@"Vollkorn-Regular" size:16.0f]];
//    [TextView3 setEditable:NO];
//    [TextView3 setScrollEnabled:NO];
//    [TextView3 setUserInteractionEnabled:YES];
//    [TextView3 setTextColor:UIColorFromRGB(0x776948)];
//    [BackUpperView addSubview:TextView3];
//    TapGesture=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectThisMessage:)];
//    [TextView3 addGestureRecognizer:TapGesture];
//    
//    CGRect NoneRect=CGRectMake(5.0f, 320.0f, 280.0f, 80.0f);
//    UILabel *nonelabel=[[UILabel alloc] initWithFrame:NoneRect];
//    [nonelabel setText:@"(No Message, just a personalized note)"];
//    [nonelabel setTextAlignment: NSTextAlignmentCenter];
//    [nonelabel setLineBreakMode:NSLineBreakByWordWrapping];
...
//    [nonelabel setUserInteractionEnabled:YES];
//    [nonelabel setTextColor:UIColorFromRGB(0x776948)];
//    [BackUpperView addSubview:nonelabel];
//    TapGesture=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(ClearPreviousSelectedMessage)];
//    [nonelabel addGestureRecognizer:TapGesture];
}
...
-(void)CustomMessagesWritten
{
    ...
    if([[CustomMessageView text] isEqualToString:@""])
    {
        ...
        [TVWriteMessage setBackgroundColor:[UIColor whiteColor]];
    }
    else
    {
